...
export default function CartSummaryItem(props) {
  const { name, subTotal, itemCount, shortDescription } = props.product;
  const image = props.product.image.split(',');
  const itemTitle = parseInt(itemCount) === 1 ? '1 Item' : `${itemCount} Items`;
    ...
    <div className='card my-1 my-sm-2'>
      <div className='row align-items-center m-0'>
        ...
        <div className='col-9'>
          ...
          <p className='card-text text-success mb-0 mb-sm-2'>
            ${subTotal} ({itemTitle})
          </p>
    ...
    <div className='card checkout-card-container my-1 my-sm-2 border-0'>
      <div className='row align-items-center m-0'>
        <div className='col-2 pl-0 pr-2 cart-item'>
          <span className='quantity-badge'>{itemCount}</span>
          <img
            src={image[0]}
            className='card-img-top'
            alt={name}
            style={{ maxWidth: '2.5rem', maxHeight: '2.5rem', objectFit: 'contain', border: '1px solid gray' }}
          />
        </div>
        ...
        <div className='col-2 col-sm-3 px-0'>
          <p className='card-text text-muted text-right text-nowrap'>${subTotal}</p>
        </div>
      </div>
    </div>
  );
